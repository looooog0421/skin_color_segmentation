...
class EllipsoidTool:
    ...
    def getPoint(self, P=None, tolerance=0.01):
        P = np.array(P)
        (N, d) = np.array(P).shape
        d = float(d)
        ...
        while err > tolerance:
            ...
            if is_invertible(V):
                ...
                return ellipsoid_point, flag
            else:
                flag = 0
                print("出于以下原因,显示的点云为原始点云:1.手掌距离摄像头过近,过远;2.摄像区域内存在多个具有'肤色'的物体")
                # print(type(P))
                return P, flag
